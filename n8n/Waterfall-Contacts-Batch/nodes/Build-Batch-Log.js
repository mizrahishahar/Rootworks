...
// for a writer batch, "<parentExecId>-<n>-ark" for that batch's AI-Ark pass; Automation
...
// pass row landed (the writer's held state stays out of it: it is for the ark pass, not the log).
...
// otherwise. Written before Batch Response answers. The key is unique by construction: batchNum is
// dealt once by the parent's Make Batches, one ark pass per batch, the parent execution id is n8n's.
...
const head=isArk?('AI-Ark pass for batch '+num(s.batchNum)+' of '+num(s.batchCount)+': '+companiesIn+' companies, '+num(s.underCap)+' still under cap, '+num(s.written)+' people written, '+covered+' of '+companiesIn+' covered after it ('+pct+'%)'):('Batch '+num(s.batchNum)+' of '+num(s.batchCount)+': '+companiesIn+' companies in, '+num(s.written)+' people written, '+covered+' of '+companiesIn+' covered ('+pct+'%)');
...
if(isArk){
  ...
  lines.push('- **Exports:** '+num((s.aiark||{}).called)+' submitted at one per second, '+num(s.arkDone)+' settled ('+num(s.arkCallbacks)+' by webhook, '+num(s.arkPolled)+' by a statistics read), '+num(s.arkWithPeople)+' with people (results fetched only for those)');
}else{
