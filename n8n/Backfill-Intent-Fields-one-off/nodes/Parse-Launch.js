...
// passes = which fills to run, any of: jobs, company, contacts, role, campaigns. Default: all.
...
const cfg={ client:String(b.client||''), base:String(b.base||''), table:String(b.table||''), datasetIds:list(b.datasetIds), roles:list(b.roles), email_campaign:String(b.email_campaign||''),
  do:{ jobs: all||passes.includes('jobs'), company: all||passes.includes('company'), contacts: all||passes.includes('contacts'), role: all||passes.includes('role'), campaigns: all||passes.includes('campaigns') } };
const missing=[];
