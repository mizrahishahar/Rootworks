...
| Found : Never Contacted | relevance = 1 and Status = done and (Messages Sent = 0 or Messages Sent is empty). Build the OR group by hand: the filter interface does not treat a blank Messages Sent as 0 the way the formula below does. | `AND({relevance} = 1, {Status} = "done", {Messages Sent} = 0)` | first_name, last_name, Title, Company, Description, Domain, Final Email, LinkedIn URL, Tag, Campaigns, Campaign Status, Messages Sent, Last Contacted, Build Date | Build Date desc |
