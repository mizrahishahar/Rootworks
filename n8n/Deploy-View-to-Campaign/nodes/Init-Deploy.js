...
// The launch names its table (People or Companies, blank = People); Resolve Table turns that into the id.
const D={launchId:r.id||'', clientId:clientIds[0]||'', table:String((f['Table']&&f['Table'].name)||f['Table']||'').trim()||'People', tableId:'', view:String(f['View']||'').trim(), target:String(f['Target']||'').trim(), dedupe:String(f['Dedupe Mode']||'Strict').trim()||'Strict', ws:'', crBase:'', clientName:'', tableName:'', campName:'', receiptName:'', errors:[], warnings:[], counters:[], uploadedNew:0, deployed:0, missing:0, rowsTotal:0, rows:{}, emailToRow:{}, inCamp:{}, skipCounts:{}, rbFailed:false, abort:null, startedAt:Date.now(), runAt:$now.toISO()};
