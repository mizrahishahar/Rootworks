...
- **You don't create adjudication fields to make a view work.** A segment view filters on NATIVE fields the waterfall itself fills - Title, Seniority, Department, Social, dates - so every future row self-sorts into its view the moment it lands. A hand-stamped judgment field (a Verdict, a Keep/No) freezes the segment at the moment of stamping: new rows arrive blank and silently vanish from every view built on it. If a split genuinely cannot be expressed on native fields, say so and hand the Operator the choice - never quietly mint a field.
