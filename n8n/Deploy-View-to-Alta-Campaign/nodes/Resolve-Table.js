...
// from the base schema just read, never from the Hub (ClayRoots Standard, law 3). The schema
...
if(!D.abort){
  ...
  if(!tables.length){ D.abort='could not read base schema'; D.errors.push('could not read base schema for '+D.crBase+(j.error?': '+JSON.stringify(j.error).slice(0,150):'')); }
    ...
    else { D.abort='table not found'; D.errors.push('Base '+D.crBase+' has no '+want+' table. Scaffold the base first. Nothing was spent or written.'); }
  }
}
return [{json:j}];
