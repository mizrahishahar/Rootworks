...
// the review signal that triggered it, the company's DiscoLike firmographics, the ICP reason
// and the in-role verdict.
//
...
// code decision.
//
// Reused from Handle Hiring Intent Signal; the ONLY diff is baseRow: the `Job ...` payload
// columns are replaced by the `Review ...` / `Trustpilot ...` payload from the source-parse.
...
// Companies: Apply ICP rows (company + biz + reason), Contact Calls rows (in-role + bodies).
const companies={};
...
function baseRow(d){
  const e=companies[d]||{}; const c=e.company||{}; const b=e.biz||{}; const s=c.signal||{}; const addr=b.address||{}; const cc=callByDomain[d]||{};
  return {
    'Domain': d,
    ...
    'Review Count': s.count||0, 'Review Latest': s.latest_date||null, 'Review Link': s.latest_url||'',
    'Review Titles': s.titles||'', 'Review Quotes': s.quotes||'', 'Review Replied': s.replied_label||'',
    'Trustpilot Rating': (s.trust_score!==undefined&&s.trust_score!==null)?s.trust_score:null,
    'Trustpilot Reviews Total': (s.total_reviews!==undefined&&s.total_reviews!==null)?s.total_reviews:null,
