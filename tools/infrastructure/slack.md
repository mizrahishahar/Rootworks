...
The `Thread ID` (a message ts) stored on the Prospect row in the Hub ([[Readme]]) ties a lead to its Slack thread; the moveplnr sync posts BDR call updates from Slack onto that row as record comments. Never post to a client channel without approval.
