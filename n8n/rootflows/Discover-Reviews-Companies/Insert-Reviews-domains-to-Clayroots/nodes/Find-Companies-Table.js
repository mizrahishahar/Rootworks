...
// Nothing here ever creates a table; the scaffold is Create Client Rootworks Infrastructure's job
// (List Building 2.0). The columns, DNC and the upsert are the helper Insert domains to Clayroots'
// business (Operator ruling 2026-09-02); this node only names the table for the log and describes
// the mirror the Signals link points at.
//
// REFUSALS, not throws (ruled 2026-09-10): a base that is not ready is an expected outcome and the
// Operator must be able to read why on a Hub row. Every guard RETURNS `refused` with the reason;
// Resolve Mirror Row carries it through and Mirror OK? routes it to Build Guard Fail Log. Nothing
// is spent and nothing is written on that path.
//
...
const refuse=(reason)=>[{ json:{ refused:reason, guard:'base', base, tableId:'', tableName:'Companies', signalsTableId:'', signalsTableName:'', mirrorHasClient:false, signalName:String(cfg.play_name||'').trim(), mirrorFilter:'' } }];
if(!tables) return refuse('Could not read the table list for base '+base+': '+JSON.stringify(body).slice(0,200)+'. Nothing was spent or written.');
const t=tables.find(x=>String(x.name||'').trim().toLowerCase()==='companies');
if(!t) return refuse('Base '+base+' has no Companies table. Scaffold the base (Create Client Rootworks Infrastructure) first. Nothing was spent or written.');
const mirror=tables.find(x=>/signals$/i.test(String(x.name||'').trim()));
if(!mirror) return refuse('Base '+base+' has no Signals mirror (a synced table whose name ends with "Signals"). Sync the Hub Signals view into the client base first. Nothing was spent or written.');
const mirrorFields=new Set((mirror.fields||[]).map(x=>x.name));
if(!mirrorFields.has('Name')) return refuse('The Signals mirror '+mirror.name+' ('+mirror.id+') has no "Name" field, so a Hub signal cannot be resolved to a mirror row. Include Name in the synced Signals view first. Nothing was spent or written.');
const signalName=String(cfg.play_name||'').trim();
if(!signalName) return refuse('The Hub signal '+cfg.signal_row+' has no Name, so there is nothing to match against the Signals mirror '+mirror.name+' ('+mirror.id+') in base '+base+'. Name the Signals row first. Nothing was spent or written.');
