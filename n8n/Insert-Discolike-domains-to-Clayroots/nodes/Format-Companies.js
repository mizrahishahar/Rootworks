...
// register's shape. Only fields the table carries are written (an extra key 422s the
// upsert). Employees accepts both a band ("51-200") and a number. Rows with no domain
// are counted and dropped; duplicate domains within the pull collapse to one.
const cfg=$('Find Companies Table').first().json;
const have=new Set(cfg.fieldNames||[]);
