...
- **Alta renders sequence variables only from defined prospect fields**, not from `extraInfoData` (a storage blob). Fields `job_needs` / `mechanical_work` / `workload` exist on the account; sending them as renderable `customFields` is parked (Operator 2026-09-01).
