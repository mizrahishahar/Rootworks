...
// expired). Everything else is what the callbacks already settled, and the rate governor's
// counters (planned, rateLimited, backoffs, unserved, stoppedEarly) ride through to the log. A
// field dropped from `out` never reaches the run row, so add here first. Ark Result Items reads
...
st=st||{ pending:[], done:[], toPoll:[], errors:[], submitted:0, planned:0, rateLimited:0, unserved:0, stoppedEarly:false, backoffs:0, attempts:0, callbacks:0 };
const out={ pending:[], done:(st.done||[]).slice(), toPoll:[], errors:(st.errors||[]).slice(), submitted:st.submitted||0, planned:st.planned||0, rateLimited:st.rateLimited||0, unserved:st.unserved||0, stoppedEarly:!!st.stoppedEarly, backoffs:st.backoffs||0, submittedAt:st.submittedAt, attempts:st.attempts||0, callbacks:st.callbacks||0, polled:polled.length };
