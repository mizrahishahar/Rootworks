...
- **An active campaign can swallow every pull-in.** *16 Sep 2026, Dave.io SOC 2 Decision Makers (c42e3694):* the webhook URL matched the one in the Alta UI, the campaign read active, and every push (300 by the deploy, then one by hand) answered 200 "Prospect uploaded successfully" and created no prospect. The same push shape lands on other campaigns. Before feeding a new Alta campaign, push ONE row by hand and confirm it with `list_prospects(campaignId)`; if it never appears, upload the list by CSV, stamp the rows on the client base by hand, and send the case to Alta support.
- **The deploy's readback is not evidence either way.** *16 Sep 2026.* It runs 90 seconds after the push and calls an archived endpoint, so it reports "landed 0" and "campaign shows no prospects at all" even on a campaign holding 352. Some pushes on a working campaign showed up hours later, in bursts. Judge landing only with `list_prospects(campaignId)` and `createdAt`.
