...
- **An inactive workspace refuses every call, reads and moves out included.** *15 Sep 2026, CaaB:* Flowroots X Piper AI was deactivated to free a slot for the new CaaB workspace while CaaB's 60 inboxes still sat in it; the move and even `list_tags` answered "Workspace is inactive. Operation not allowed." Move every inbox out before a workspace is deactivated; the UI's own move still works out of an inactive workspace when the API will not.
- **Warmup slow ramp switches itself off, one batch at a time, with no cause we can pin.** *15-16 Sep 2026, CaaB:* after a UI workspace move, 10 inboxes of one batch read back `warmup_rampup.is_slow_rampup: false`. Fixed, then the next day, after only a signature bulk update, 10 inboxes of a different batch had flipped the same way; the fixed ten stayed on. Every other field was intact both times. So never trust the flag between reads: diff the whole payload against the last snapshot after any write or move. `bulk_update_email_accounts` with `bulk_warmup_is_slow_rampup: yes`, `warmup_initial_daily_limit`, `warmup_pace_increment` and `warmup_max_daily_limit` (that batch's own value) restores them without touching anything else.
...
- **`allocate_inboxes_by_tag` can ignore its inputs.** *16 Sep 2026, Adelante:* two calls, one with `input` as a JSON string and one with `client`/`tag` as fields, both answered "no Hub client named string" and changed nothing. Fallback: read each campaign's list with `get_campaign_email_accounts`, build the id list, patch it (below), and confirm `email_accounts_count` on `list_campaigns`.
- **The active tag is not the campaign sender list.** *16 Sep 2026, Adelante:* campaigns carried 83 of 93 active inboxes on purpose (no Hebrew senders on English campaigns, two flagged domains held out). Add new inboxes to the current list; never overwrite it with the whole tag.
