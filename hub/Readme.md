...
- **Meetings** (`tblAo6SfTPTOJrnbX`) - every recorded meeting. `Meeting Type` splits the two worlds: `sales` rows link `Company` (a Prospects row, pre-deal); `client` rows link `Client` (the registry, post-deal check-ins and working sessions). The Fathom sync routes by the registry's `Email Domains` and only sales meetings move `PipelineStatus`. Summary carries action items; Transcript holds the full text (99k cap).
- **Reports, Openers, Lead Lists, Messages, Contacts** - what their names say; Transactions / Recurring Templates / Content Posts are HQ finance and content, not fulfillment.
