...
// names, the domain, and the candidate list built from the Email cell (every address the contact
// providers delivered, in the order they were merged; a foreign-domain address is kept last). Per
// domain: the address pattern the base already proves (from Status = done rows at that domain whose
// Final Email matches one of the known shapes for that person: the majority shape, at least one
// proof) and whether the domain is known catch-all (a done row there was verified by BounceBan).
...
const state={ order:[], rows:{}, patterns, domainCatchAll, stats:{ rows:rows.length, candidatesIn:0, knownRows:known.length, patternsKnown:Object.keys(patterns).length, mvCalls:0, lmCalls:0, lmFound:0, guesses:0, catchAllDomains:0 } };
for(const r of rows){
  const f=r.fields||{};
  ...
  state.rows[r.id]={ id:r.id, first, last, firstRaw:String(f.first_name||'').trim(), lastRaw:String(f.last_name||'').trim(), domain, candidates, resolved:null, catchAll:!!domainCatchAll[domain], guessed:false, lmAsked:false, error:'' };
}
