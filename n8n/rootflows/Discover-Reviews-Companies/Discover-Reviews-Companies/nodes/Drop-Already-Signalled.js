...
// Filter & Qualify Reviews dedupes inside one scrape (one row per domain). This node dedupes
// against the base: a company whose Companies row already links THIS signal's mirror row has been
// through the whole chain before and is dropped here, so DiscoLike is not paid to judge it again
// and it does not re-enter the queue. A company already in the base under a different signal, or
// under none, passes. Read Known Signals is one Airtable read by Domain over the qualified domains.
// The kept items keep Filter & Qualify Reviews' shape byte for byte: Company Facts aligns BizData
// answers to this list by index. Same node as in Insert Hiring domains to Clayroots.
...
const items=$('Filter & Qualify Reviews').all();
