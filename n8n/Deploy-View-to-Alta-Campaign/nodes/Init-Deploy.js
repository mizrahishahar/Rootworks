...
const D={launchId:r.id||'', clientId:(cid&&typeof cid==='object')?String(cid.id||''):String(cid||''), table:String((f['Table']&&f['Table'].name)||f['Table']||'').trim(), tableId:'', view:String(f['View']||'').trim(), target:String(f['Target']||'').trim(), ws:'', crBase:'', clientName:'', tableName:'', campName:'', pullInUrl:'', mirrorTableId:'', stampMirrorRid:'', shareViewLink:'', errors:[], warnings:[], skipCounts:{}, rows:{}, rowsTotal:0, pushed:0, landed:0, missing:0, pausedTitle:0, abort:null, startedAt:Date.now(), runAt:$now.toISO()};
