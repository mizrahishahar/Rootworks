// The Hub contact leg's row, one per call, per the logging standard: computed Status, failed[] ->
// Errors, skips separated, Client attached only when the run serves exactly one client.
const P = $('Contact Params').first().json;
...
lines.push('', 'The Hub contact door, not the Rootflow: one Hub Prospects row, answered synchronously for the lead handlers.');
const row = {
