...
// (the PlusVibe campaign id), Dedupe Mode. No defaults (Operator ruling 2026-09-02): a launch row
// that leaves Table or View blank is refused here, before anything is read or sent.
const sd=$getWorkflowStaticData('global'); const dk='deploy_'+$execution.id;
const r=($input.first()||{}).json||{};
const f=r.fields||r;
const clientIds=Array.isArray(f['Client'])?f['Client']:[];
const D={launchId:r.id||'', clientId:clientIds[0]||'', table:String((f['Table']&&f['Table'].name)||f['Table']||'').trim(), tableId:'', view:String(f['View']||'').trim(), target:String(f['Target']||'').trim(), dedupe:String(f['Dedupe Mode']||'Strict').trim()||'Strict', ws:'', crBase:'', clientName:'', tableName:'', campName:'', receiptName:'', errors:[], warnings:[], counters:[], uploadedNew:0, deployed:0, missing:0, rowsTotal:0, rows:{}, emailToRow:{}, inCamp:{}, skipCounts:{}, rbFailed:false, abort:null, startedAt:Date.now(), runAt:$now.toISO()};
