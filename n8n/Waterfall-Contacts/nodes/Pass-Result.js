...
// every paid call died: no further batches, straight to the wait for the ark rows already in
// flight. fireArk = this batch gets its AI-Ark pass now (closed, AI-Ark on, not stopped): Ark
// Batch Item shapes it and Fire Ark Pass launches it without waiting.
const p=$('Launch Params').first().json;
let batchNum=0; try{ batchNum=Number($('Loop Batches').first().json.batchNum)||0; }catch(e){}
const j=$input.first().json||{};
const out={ mode:'writer', batchNum:batchNum, key:String(batchNum), status:'closed', written:0, allFailed:false, stop:false, fireArk:false, reason:'' };
...
out.fireArk=out.status==='closed'&&(p.sources||[]).indexOf('AI-Ark')>-1;
return [{ json: out }];
