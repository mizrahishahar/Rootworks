...
Compiled from the live Flowroots Hub (`appQG6dK0FIOhTxOl`) by `scripts/hub-pull.js`. Do not hand-edit.
...
| HeyReach API Key | `fldelD3SJGUv5J6Rj` | singleLineText | The client's HeyReach workspace API key (HeyReach -> Settings -> API). Read live by every HeyReach machine (the deploy door, the reply intake, the campaign sync, the ops door) as the X-API-KEY header. One workspace per client. Empty = this client has no HeyReach and the HeyReach lane refuses the run before anything is read or sent. Pasted by the Operator, never by a machine. |
| Infra Reports to Client | `fldJUmIPCZZrkLzvJ` | checkbox | Switched on by the Operator only. When checked, the Inbox Manager also prepares this client's infrastructure emergencies and Monday report in client wording, as drafts for the Operator's approval before anything reaches the client channel (Slack Channel ID). Empty = internal only. Standard: standards/infrastructure.md, To the client. |
...
| Seniority | `fldtL8kLruxOJabAA` | multipleSelects | RETIRED 2026-09-02. Never read by any machine. Superseded by Roles, which carries the full People.Seniority vocabulary. Safe to delete. - Choices: C-Suite, Founder, Owner, President, VP, Head, Director, Manager |
...
| Contact | `fldnJWucAHpx2PgV6` | multipleRecordLinks | Read by two machines, each only on its own launch rows. Waterfall Phones: set it to run the waterfall on ONE contact instead of sweeping a table (the launch leg fires just this row and ignores Table ID / View / Max Rows); leave empty for a normal sweep. Create Client Rootworks Infrastructure: the new client's own contacts, whose email addresses get writer access to the client's Drive Shared folder. Empty there means nobody is invited and the run says so. |
...
| Sources | `fldNHjtxyFbWPVCD5` | multipleSelects | Waterfall Contacts: which contact sources this run may call. Blank means all three. SUPERSEDED 2026-09-02 by Tiers, the named-mode single select. Still read as the fallback when Tiers is blank, so nothing in flight breaks; delete it once no launch row relies on it. - Choices: ContaGen, Supersoniq, AI-Ark |
...
| Extras | `fldUzNoF5hN2LkbnV` | multipleSelects | Scaffold Client Base and Create Client Rootworks Infrastructure: the declared extras groups to create on Companies and People for this client. The CLAYROOTS SCHEMA template deliberately carries none, so a client's groups are picked here at onboarding. Working machines never create columns; they refuse a base that lacks their group. - Choices: Storeleads, Hiring, Reviews |
| Tiers | `fldMg1W5uqWocEPSe` | singleSelect | Waterfall Contacts: which tiers this run may call, as one named mode. "ContaGen -> Supersoniq -> AI-Ark" is the full waterfall. "AI-Ark" is the intent path: AI-Ark only, a flat cap of 5 people per company and the buyer seniorities only (founder, owner, partner, c_suite, vp). "ContaGen -> Supersoniq" skips AI-Ark. Blank falls back to the Sources multi-select, and blank on both is the full waterfall. Supersedes Sources, which can be deleted once nothing reads it. - Choices: ContaGen -> Supersoniq -> AI-Ark, AI-Ark, ContaGen -> Supersoniq |
| Roles | `fld0NcoXexQgV4iiw` | multipleSelects | Waterfall Contacts: the seniority net each source is asked for. Blank leaves every source on its in-code default. A pick is mapped into each provider's own vocabulary and OVERRIDES that default; a role with no home at a provider is dropped there and named in the run log. Same vocabulary as People.Seniority. Replaces the never-read Seniority field. - Choices: Founder, Owner, Partner, Board / Chair, C-Suite, President, Executive, VP, EVP / SVP, Head, Director, Manager, Senior, Unclassified |
| Clayroots Base ID | `fldo8GCDYhX7cvKsY` | singleLineText | Create Client Rootworks Infrastructure: the base id (appXXXXXXXXXXXXXX) of the client's freshly duplicated CLAYROOTS SCHEMA template base. Required on that machine's launch row; the run refuses before it touches Drive or Slack when it is missing or malformed. Every other machine resolves the base from the Clients row instead, because by then the client exists in the registry. |
| Prompt | `fldI22CVadsAAdFvw` | multilineText | Enrich Discogen Research: what DiscoGen is asked about each row of the view. Free form: a question, an instruction, a line to write. With Evidence on, the machine appends the structured-return instruction itself. |
| Output Field | `fldZ4CIRgOEOoCnPO` | singleLineText | Enrich Discogen Research: the column on the client's Companies table this run creates (on first use) and writes. One run, one column. With Evidence on, '<Output Field> Evidence' and '<Output Field> Confidence' ride beside it. |
| Output Type | `fldLnk9NsbiqkEzlN` | singleSelect | Enrich Discogen Research: the Airtable type the Output Field column is created as when the base lacks it. Long text (default) for writing; Text for a short value; Number; Checkbox for yes/no only; Single select for yes/no/unknown or a category (filterable). - Choices: Long text, Text, Number, Checkbox, Single select |
| Web Search | `fldGm4jIPlfxUzg3j` | checkbox | Enrich Discogen Research: off (default) means DiscoGen answers from the company profile and homepage text. On means it also searches the web through the account's default search provider; costs more, use when the answer is not on their own site. |
| Evidence | `fldzZVbRw9RbDFO2B` | checkbox | Enrich Discogen Research: off (default) writes DiscoGen's answer verbatim into Output Field. On asks for a structured return and writes three columns: the answer, '<Output Field> Evidence' (a short quote from the source) and '<Output Field> Confidence' (0 to 1); an 'unknown' answer is written blank so a rerun picks it up. Use for constrained answers (yes/no, a category, a number), never for a generative prompt. |
| Overwrite | `flddXjScFiLm7VWSC` | checkbox | Enrich Discogen Research: off (default) fills blanks only, a row already holding a value in Output Field is skipped and counted. On asks every row in the view again and rewrites the column. |
| Left in View | `fld7rapMCXQw9l3PR` | number | Deploy doors. After the run: rows the view could still give (after DNC, the Campaigns stamp and the variable check) minus what this run sent. Zero means the view is dry. The same number is stamped on the campaign row (Campaigns.Left in View), which is where the Campaigns Manager reads it; here it is the run's own record. |
...
| Sequencer | `fldiDh8kyaOhcDM0t` | singleSelect | Choices: PlusVibe, Alta, HeyReach, Email Bison |
...
| Live View ID | `fldj1RB3PRR8Cb76m` | singleLineText | The Airtable view id (viw...) this campaign drinks from, on the table named by Table (People or Companies) in the client's ClayRoots base. Set it and the daily feed enrolls from that view every day; clear it and the campaign is fed only by hand. The view owns dedupe: it must exclude rows already enrolled. Source does not matter, a signal queue, a drip slice or a hand-built list all feed the same way. Replaces the Signal link plus Signals.View coupling, retired 2026-09-02. |
| ContactList | `fldlget9B3kkWtTof` | multipleLookupValues |  |
| CompaniesList | `fldxRVI7QeZXPtX71` | multipleLookupValues |  |
| Stage | `fldOEzL4gFsv847yo` | singleSelect | Campaigns Manager. Test, Scale, Run, Killed. The Operator sets Test when binding a Live View ID; the manager moves Test to Scale (line reached, positives met) or Test to Killed (line reached, none); the Operator sets Run or Killed from Scale. A campaign with no Stage is not managed. - Choices: Test, Scale, Run, Killed |
| Left in View | `fldbADgt52kOdE59d` | number | Feeding state, written by the deploy door at the end of every run on this campaign: how many rows the Live View still held after the run. 0 means the view is dry. The manager reads it from here, never from the run log. |
| Last Fed | `fldwSl6cDU4sjreps` | dateTime | Feeding state, written by the deploy door at the end of every run on this campaign: when it last ran. The manager feeds a campaign once a day, judged by this. |
...
| Campaigns | `fldNSaJ1fsF4QGGlT` | singleLineText |  |
...
| View | `fldOtjp7UF0pXLpCP` | singleLineText | The Airtable VIEW ID (viw...) of the queue view this signal feeds, on the client table named by the linked Campaign's Table (People for a lead feed, Companies for a company feed). Read by the deploy doors' daily feed. Replaces Campaigns.Signal View (deleted 2026-09-02). Empty = the feed refuses this signal by name in its log. |
