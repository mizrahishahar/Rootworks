...
- **The backend** (n8n) does the heavy work, and you hold its entire source as readable code: [n8n/INDEX.md](n8n/INDEX.md) lists every automation and when to use it, grouped as the repo is: `n8n/rootflows/<Rootflow>/` (one folder per Rootflow, its `rootflow.json` declaration beside its workflows), `n8n/rootworks/` (the Hub machines, the lead handlers, the deploys), `n8n/helpers/`, `n8n/archive/`. The full source of each sits there, maintained by `scripts/n8n-pull.js`. When you need to know exactly what a run will do, read it. `n8n/layout.json` is the approved list: a workflow exists in n8n only with a line there, written after the Operator approved it; never create one otherwise.
