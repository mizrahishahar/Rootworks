// Build Log: this run's Hub row. A launched run updates its launch row; an event run (fired by
// Enrich Emails) is keyed "<caller execution>-verify" so it reads as a detail row under the run it
// belongs to. Status computed; an address BounceBan refused at submit is an error (its row stays
// verifying, untouched); a row still pending at the cap is a skip (untouched too; a manual launch
// takes it).
...
let st={ rows:0, done:0, noEmail:0, pending:0, refusedRows:0, byProvider:{}, submitted:0, refused:0, verdicts:{} };
...
const scope=(p.rowIds&&p.rowIds.length)?('the '+p.rowIds.length+' rows run '+p.parentExecId+' just marked verifying'):('every row with Status = verifying in People '+(t.viewName||'(whole table)'));
const lines=[
