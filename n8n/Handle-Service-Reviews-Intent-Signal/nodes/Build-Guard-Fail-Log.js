...
// Reused from Handle Hiring Intent Signal; diffs: the Automation name and the fix-it text
// naming the Signals row instead of the retired play grammar.
let g={};
...
const row={
 ...
 'Records Out': 0,
 'Errors': 1,
 'Target': g.table||'(no table)',
 'Trigger':'event',
 'Execution ID': String($execution.id),
 'Execution Link': 'https://n8n.flowroots.com/workflow/'+$workflow.id+'/executions/'+$execution.id,
 'Description': '**Guard tripped before any paid call, 1 error**\n\n**Where:** '+what+'\n\n**Missing / unreadable:** '+missing+'\n\nThe Signals row needs: Client link, Target Table (tbl id), Roles, Country, Max Employees, ICP. The Apify payload needs {play: <Signals record id>, resource}. Fix and rerun.'
};
