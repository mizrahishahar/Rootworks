...
// push inlines as REGISTER at the @@register line: Deploy Error on every deployed or skipped row;
...
if(D&&!D.abort){
  ...
  const NEEDS=['Deploy Error'].concat(p.mirrorTableId?['Campaigns']:[]).map(n=>{ const f=T.fields.find(x=>x.name===n); if(!f) throw new Error('Check Columns: the register has no field "'+n+'" on Companies'); return f.name; });
