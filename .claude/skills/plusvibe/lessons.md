...
- **A workspace move can silently switch warmup slow ramp off.** *15 Sep 2026, CaaB:* after a UI move of 60 inboxes, 10 of one batch read back `warmup_rampup.is_slow_rampup: false`, every other field intact. Diff the whole payload against a pre-move snapshot after every move; `bulk_update_email_accounts` with `bulk_warmup_is_slow_rampup: yes`, `warmup_initial_daily_limit`, `warmup_pace_increment` and `warmup_max_daily_limit` restored them.
