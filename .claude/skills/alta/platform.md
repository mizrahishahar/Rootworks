Teaches: the whole Alta book: what it owns, the MCP, working the inbox, campaigns, reps and limits, opt-outs, numbers, and the dated lessons paid for.
...
- **Sessions:** the Alta MCP. `list_*` / `get_*` to read, `send_linkedin_message` / `send_email_message` to reply, `assign_tags`, `*_communication_opt_outs`, `pause_*` / `resume_*`, and `ask_alta_agent` for numbers.
...
- **Pause the campaign before working its threads.** See the lessons.
