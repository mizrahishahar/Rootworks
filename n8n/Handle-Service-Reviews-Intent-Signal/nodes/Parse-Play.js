...
// 'service_reviews'), so a launch whose id went stale still resolves as long as exactly ONE
// service_reviews signal exists; ambiguity or no match stops the run before any paid call.
...
// Diff from the hiring handler: the fallback Signal Type string and the default event_type.
...
let legacy=false;
if(!hit){
  const same=all.filter(r=>String(r.f['Signal Type']||'')==='service_reviews');
  if(same.length===1){ hit=same[0]; legacy=true; }
  else missing.push(same.length===0?'no Signals row found for launch id '+wanted:'launch id '+wanted+' unknown and '+same.length+' service_reviews signals exist (ambiguous); update the Apify webhook payload to the Signals record id');
}
const f=hit?hit.f:{};
...
return [{ json: {
  ...
  client, table, event_type: String(f['Signal Type']||'service_reviews'),
