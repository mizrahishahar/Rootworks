// Handoff Tick: closes one hand-off (after every writer batch, and once more after the last ark
// row landed) with the live Running row. This hand-off's outcome is read from the node's own
...
// The run so far is read from every Pass Result run, every Ark Batch Item run and every Fire
// Waterfall run (run by run, until a run index does not exist), and from Ark Rows Check once it
// has run. No Tally here, no read-add-write (ruled 2026-09-02): the pass rows carry the counters,
// the close reads them by prefix. The item out is the row, nothing else, so Stamp Progress can
// auto-map it; the gates after it read Pass Result and Ark Rows Check.
...
const arkFired=runs('Ark Batch Item').length;
...
const stage=closed+' of '+batchCount+' writer batches closed ('+batches.length+' launched, one at a time), '+arkFired+' AI-Ark pass'+(arkFired===1?'':'es')+' fired'+(ark?(', '+ark.landed+' of '+ark.expected+' landed'+(ark.timedOut?' (wait timed out)':'')):'');
const out={
