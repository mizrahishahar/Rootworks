...
// @@register line: the register core Build Companies maps, by name, plus the Hiring signal's
// declared extras, by group (the scaffold creates them when Hiring is picked on its launch row).
...
if(missing.length){ throw new Error('Table "'+cfg.tableName+'" ('+cfg.tableId+') in base '+cfg.base+' is missing the columns Handle Hiring Intent Signal writes: '+missing.join(', ')+'. Scaffold the base (Scaffold Client Base) first. Nothing was spent or written.'); }
